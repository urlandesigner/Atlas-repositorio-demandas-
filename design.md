version: alpha
name: Cursor
description: An AI-first code editor whose marketing site reads like a quietly-confident developer-tools brand with a warm-cream editorial canvas (`#f7f7f4`) instead of the typical dark IDE atmosphere. Near-black warm ink (`#26251e`) carries body and display alike — display sits at weight 400 with negative letter-spacing for a magazine feel rather than a bold tech voice. The single brand voltage is **Primary Blue** (`#3A4ADF`) reserved for primary CTAs and the wordmark. A signature pastel timeline palette (peach, mint, blue, lavender, gold) marks AI-action stages (Thinking / Reading / Editing / Grepping / Done) — only inside in-product timeline visualizations. Cards use minimal hairlines, no shadows, generous 80px section rhythm. CursorGothic for display/body, JetBrains Mono on every code surface (which is roughly half the page).

colors:
  primary: "#3A4ADF"
  primary-active: "#2F3FC3"
  ink: "#26251e"
  body: "#5a5852"
  body-strong: "#26251e"
  muted: "#807d72"
...
Cursor's marketing site reads as a quietly-confident developer brand that believes in editorial calm over IDE-darkness. The base canvas is **warm cream** (`{colors.canvas}` — #f7f7f4) holding warm near-black ink (`{colors.ink}` — #26251e) for body and display alike. The single brand voltage is **Primary Blue** (`{colors.primary}` — #3A4ADF) reserved for primary CTAs and the wordmark — used scarcely.
...
- Single CTA color: `{colors.primary}` (Primary Blue #3A4ADF). Used scarcely.
...
### Brand & Accent
- **Primary Blue** (`{colors.primary}` — #3A4ADF): Primary CTA pills, wordmark, hero accent. Used scarcely.
- **Primary Blue Active** (`{colors.primary-active}` — #2F3FC3): Press state.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on Primary Blue.
...
**`button-primary`** — The signature Primary Blue CTA. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}` (14px / 500), padding 10px × 18px, height 40px, rounded `{rounded.md}` (8px).
...
**`cta-band`** — Pre-footer "Try Cursor now" band. Background `{colors.canvas}`, centered display headline in `{typography.display-lg}`, single Primary Blue CTA. 96px vertical padding.
...
### Do
- Reserve `{colors.primary}` (Primary Blue) for primary CTAs and brand wordmark.
...
### Don't
- Don't introduce a secondary brand action color. Primary Blue is the only one.
